...
## DEC-062 — حذف کامل مسیر Excel؛ افزودن بازی فقط با فرم داخل سایت

Decision ID: DEC-062
Date: 2026-09-22
Status: APPROVED BY OWNER / IMPLEMENTED

Context: مالک تصریح کرد: «کلا در همه‌ی بخش‌ها در افزودن بازی، تمپلیت اکسل و امکان اضافه کردن بازی با اکسل رو حذف کن. حتی در بخش پایینی صفحه قسمت فایل‌های ایمپورت‌شده رو هم حذف کن. کلا فقط از فرم داخل سایت بازی رو اضافه می‌کنیم. توی اکسل ایرادهای زیادی وجود داره و تصمیم گرفتم حذفش کنم.»
Decision: روت‌های `imports/template` و `imports/excel/preview`، متدهای `previewExcel` و `template`، خواننده `TeelleXlsxGameReader`، تست `ExcelMultiValueReaderTest` و هر سه فایل `Teelle_New_Game_Template_v1/v2/v3.xlsx` حذف شدند. صفحه «افزودن بازی‌ها» فقط فرم داخلی سایت را نشان می‌دهد: بخش «افزودن از فایل Excel»، دکمه «دریافت تمپلیت Excel» و فهرست «فایل‌های ایمپورت‌شده» از رابط برداشته شدند. جریان «بررسی بازی و پیش‌نمایش → تأیید → پیش‌نویس» و بازگردانی برای بازی‌های فرم دست‌نخورده ماند و `ContentImportService::preview` (JSON) برای سیدر بازی‌های پایه حفظ شد؛ جدول‌ها و داده‌های بسته‌های قبلی حذف نشدند (قرارداد آپدیت امن). تمپلیت‌های `Teelle_Game_Review_Template_v1/v2` فقط سند مرجع تاریخی ۲۵ بازی پایه‌اند.

## DEC-060 — گزینه‌های مازاد شرایط اجرا و پلن‌های جیگری قابل ویرایش

Decision ID: DEC-060
Date: 2026-09-22
Status: APPROVED BY OWNER / IMPLEMENTED

Context: مالک خواستار سه اصلاح شد: (۱) «شرایط اجرا» (فضا، صدا، کثیفی، تعامل، مشارکت، آماده‌سازی) و «ترکیب بازیکنان» و «انرژی کودک/همراه» هم مانند سایر فیلدهای دسته‌ای امکان انتخاب چندگانه داشته باشند؛ (۲) عبارت «انتخاب چندگانه» از تیتر همه بخش‌ها حذف شود ولی قابلیت بماند؛ (۳) پنل پلن‌ها عنوان «پلن‌های جیگری» بگیرد و مدیر بتواند علاوه بر قیمت، متن و جزئیات هر پلن را ویرایش کند. همچنین تیترهای انگلیسی «Coverage Matrix» و «Jigari pricing» در پنل فارسی شدند.
Decision: برای هر فیلد دسته‌ای، منوی کشویی «مقدار اصلی» حفظ می‌شود و چک‌باکس‌های «گزینه‌های دیگر» جدا (فرم افزودن: `game[metadata_extra]`، فرم ویرایش: `metadata_extra`) ارسال و بعد از حذف مقدار اصلی، در ستون JSON `alternatives` ذخیره می‌شوند؛ سطح نظارت هم همین رفتار را دارد. در تمپلیت اکسل v3 چند مقدار ویرگولی ستون‌های شرایط اجرا و ترکیب بازیکنان likewise پذیرفته می‌شود (مقدار نخست اصلی). پنل پلن‌ها به «پلن‌های جیگری» تغییر نام یافت و ستون فقط-افزایشی `description` (تا ۵۰۰ نویسه، چندخطی) به `plans` اضافه شد؛ عنوان، توضیحات، قیمت و وضعیت نمایش همه سه پلن از همین پنل ویرایش می‌شود و تغییرات بلافاصله در صفحه عمومی تیله جیگری نمایش داده می‌شود.
Data model: مهاجرت `2026_09_21_020000_add_description_to_plans_table` فقط یک ستون nullable اضافه می‌کند و به هیچ جدول دیگری دست نمی‌زند (قرارداد آپدیت امن). موتور تطبیق قطعی هیچ تغییری نکرده و مقدار اصلی هر فیلد همچنان در ستون/pivot خودش ذخیره می‌شود؛ مقادیر مازاد صرفاً اطلاعاتی و برای بازبینی انسانی‌اند.
Consequences: صفحه عمومی جیگری به جای عبارت ثابت «همه امکانات تیله جیگری»، عنوان و توضیحات ویرایش‌شده هر پلن را نشان می‌دهد؛ عنوان پلن با تبدیل رقم به فارسی رندر می‌شود.
Affected Components: فرم‌های افزودن/ویرایش بازی، خواننده ایمپورت اکسل، GameContentWorkflow، پنل پلن‌ها (PlanController/PlanPricingManager)، صفحه عمومی تیله جیگری.
...
## DEC-058 - نگهداری محتوایی، پایداری داده در آپدیت و بازه ریست نشست

- Date: 2026-09-21
- Status: APPROVED BY OWNER / IMPLEMENTED
- Magazine: حذف دسته‌بندی مجله با شرط ایمن اضافه شد؛ دسته‌ای که مطلب یا زیرمجموعه دارد حذف نمی‌شود و ابتدا باید مطلب یا زیرمجموعه‌اش جابه‌جا شود. فیلدهای SEO فرم مطلب با میکروکپی ساده فارسی توضیح داده شدند: «عنوان در نتیجه جست‌وجو» (خالی یعنی همان عنوان مطلب) و «توضیح کوتاه زیر نتیجه جست‌وجو».
- Import: بلوک ثابت «بازی‌های پیشنهادی اولیه تیله» از صفحه افزودن بازی حذف شد و فهرست «فایل‌های ایمپورت‌شده» جای آن را گرفت؛ هر فایل Excel یا فرم بررسی‌شده در این فهرست می‌ماند تا مالک تصمیم بگیرد به پیش‌نویس برود یا بازگردانی شود. دکمه بارگذاری به «بررسی فایل و پیش‌نمایش» تغییر کرد. مسیر Pilot Preview حذف شد؛ فایل مرجع pilot برای تست باقی می‌ماند.
- Site content: پنل «متن‌ها و چیدمان» (فقط مدیر، مجوز site.manage) اکنون تیتر و معرفی «تیله جیگری» و «مجله تیله» و برچسب/ترتیب شش‌حالتی منوی بالای سایت را نیز ویرایش می‌کند. مقادیر nav_order قبلی برای سازگاری داده قدیمی همچنان رندر می‌شوند.
- Update safety: پیش‌نمایش محلی به MySQL ایزوله دائمی روی 127.0.0.1:14018 و Session/Cache دیتابیسی متصل است؛ آپدیت کد، Migration فقط-افزایشی و انقضای Session دیتابیس هیچ اثری روی حساب‌ها، پروفایل کودک و پیش‌نویس‌ها ندارد. بازنویسی دیتابیس فقط با Migration عمدی و بازبینی مالک انجام می‌شود. علت از بین رفتن پیش‌نویس‌های قبلی، دیتابیس موقت تست بود؛ این اسکریپت دائمی است و دیگری جایگزین آن نمی‌شود.
- Cache busting: فایل نسخه /teelle-asset-index.txt (مقدار TEELLE_DEPLOY_VERSION) مرجع تشخیص آپدیت است؛ Service Worker پس از هر آپدیت کش استاتیک را پس‌زمینه‌ای تازه می‌کند و در صورت تفاوت نسخه، پنجره یک‌خطی «سایت رو به‌روز کردیم؛ لطفاً صفحه را دوباره بارگذاری کنید» با دکمه بارگذاری دوباره نمایش داده می‌شود. ورود و ثبت‌نام و داده پنل کاربر با پاک‌شدن کش مرورگر حذف نمی‌شود چون Session و همه داده‌ها در دیتابیس می‌مانند.
- Session lifetime: نشست کاربران پس از بازه استاندارد هفت روز (TEELLE_SESSION_MAX_AGE_MINUTES = 10080) ریست و فقط ورود دوباره لازم می‌شود؛ ثبت‌نام و داده حساب دست‌نخورده می‌ماند. این میان‌افزار EnsureFreshSession است و با Logout واقعی کاربر تفاوت داده‌ای ندارد.

## DEC-061 - سادگی فرم‌های بازی، منع اضافه‌نویسی و جایگاه پیام آپدیت

- Date: 2026-09-22
- Status: APPROVED BY OWNER / IMPLEMENTED
- Forms: در فرم‌های افزودن و ویرایش بازی، برای همه فیلدهای دسته‌ای (سطح نظارت، شرایط اجرا، ترکیب بازیکنان و انرژی‌ها) فقط چک‌باکس هست و منوی کشویی حذف شد. نخستین چک‌باکس علامت‌خورده انتخاب اصلی است و انتخاب‌های بعدی به‌عنوان جانبی در `alternatives` ذخیره می‌شوند؛ اگر هیچ چک‌باکسی علامت نخورده باشد، مقدار ذخیره‌شده قبلی (مسیر اکسل) حفظ می‌شود و هرگز مقدار اصلی پاک نمی‌شود. موتور تطبیق تغییری نمی‌کند.
- Copy rule (مالک): هیچ عبارت اضافه‌ای در تیتر بخش‌ها نوشته نمی‌شود. «گزینه‌های دیگر» و مشابه آن از همه تیترها حذف شد؛ تیتر هر بخش فقط نام خودش است. این قاعده برای همه رابط‌های آینده الزامی است.
- Deploy notice: پیام «سایت رو به‌روز کردیم؛ لطفاً صفحه را دوباره بارگذاری کنید» یک‌خطی است، دکمه «بارگذاری» در همان خط کنار متن قرار دارد و جایگاه آن بالای سایت، بین منوی بالای سایت و تیله مرکزی است (میزبان `data-deploy-notice-host` در layout؛ اعلان PWA هم در همان جایگاه رندر می‌شود).

## DEC-059 - هماهنگی فرم بازی با تمپلیت اکسل و انتخاب چندگانه

- Date: 2026-09-21
- Status: APPROVED BY OWNER / IMPLEMENTED
- Decision: فرم «افزودن بازی» باید همه ستون‌های تمپلیت اکسل را بپوشاند: تصویر اختیاری با متن جایگزین، «اولویت محتوا» و «دلیل اولویت» به فرم و صفحه بازبینی اضافه شدند. برای تمامی آیتم‌های زیرمجموعه سطح نظارت، اجرای بازی، شرایط و دسته‌بندی، علاوه بر منوی کشوییِ مقدار اصلی، چک‌باکس همه گزینه‌ها در فرم‌های افزودن و ویرایش وجود دارد تا چند انتخاب ثبت شود.
- Data model: گزینه‌های اضافیِ چندانتخابی در ستون JSON جدید `alternatives` روی `game_facts` و اولویت در `content_priority`/`priority_reason` ذخیره می‌شوند؛ مهاجرت فقط-افزایشی است و هیچ ردیف یا ستون موجود را تغییر نمی‌دهد. انتخاب اصلی همچنان همان ساختار قبلی (pivotها و enumها) را تغذیه می‌کند و موتور تطبیق قطعی و Metadata-based هیچ تغییری نمی‌کند؛ مقادیر اضافی صرفاً اطلاعاتی و برای بازبینی انسانی‌اند.
- Excel: تمپلیت رسمی به v3 ارتقا یافت؛ راهنمای ردیف ۲ توضیح می‌دهد در ستون‌های دسته‌ای چند مقدار با ویرگول پذیرفته می‌شود (مقدار نخست انتخاب اصلی، بقیه گزینه‌های جانبی) و اعتبارسنجی تک‌انتخابی از آن ستون‌ها حذف شد. خواننده ایمپورت مقادیر ویرگولی و ستون‌های اولویت را می‌خواند و فایل‌های v2 بدون خطا همچنان پذیرفته می‌شوند. دکمه فرم به «بررسی بازی و پیش‌نمایش» تغییر کرد.
- Image: تصویر فرم (اختیاری) با همان معاینه امضای بایت‌ها و checksum تکراری در قرنطینه ذخیره و به بسته ایمپورت وصل می‌شود؛ هنگام تأیید بسته به کاور پیش‌نویس ساخته‌شده منتقل و در وضعیت قرنطینه می‌ماند تا با جریان مستقل «بازبینی رسانه» تأیید شود. بدون تصویر هم جریان کار همان مسیر قبلی است.
- Local preview: ریشه خالی‌بودن «برچسب‌ها»، «نکات ایمنی ساختاری» و «بازه سنی» اجرا نشدن سیدر تاکسونومی روی دیتابیس ایزوله محلی بود. اسکریپت پیش‌نمایش اکنون در هر اجرای وب، مهاجرت‌های فقط-افزایشی و سیدر ایدمپوتنت `SystemTaxonomySeeder` را اجرا می‌کند تا فرم‌ها هرگز گزینه خالی نبینند؛ داده‌های مالک دست‌نخورده می‌ماند.
