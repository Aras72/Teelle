...
Current Stage: DEC-060 CONDITION MULTI-SELECT COMPLETION AND EDITABLE JIGARI PLANS IMPLEMENTED AND VERIFIED
...
- DEC-060 (Maintenance، Phase 14 را باز نمی‌کند): چندانتخابی به همه فیلدهای باقی‌مانده رسید — فضای لازم، میزان صدا، کثیفی، نوع تعامل، مشارکت همراه، سختی آماده‌سازی، ترکیب بازیکنان و انرژی کودک/همراه (و سطح نظارت) اکنون علاوه بر منوی مقدار اصلی، چک‌باکس «گزینه‌های دیگر» دارند؛ انتخاب‌های مازاد جدا (`metadata_extra`) ارسال و پس از حذف مقدار اصلی در `game_facts.alternatives` ذخیره می‌شوند و موتور تطبیق تغییری نمی‌کند. عبارت «انتخاب چندگانه» از تیتر همه بخش‌های هر دو فرم حذف شد. پنل پلن‌ها به «پلن‌های جیگری» تغییر نام یافت و حالا عنوان، توضیحات (ستون فقط-افزایشی `plans.description` تا ۵۰۰ نویسه)، قیمت و وضعیت نمایش هر پلن قابل ویرایش است و بلافاصله در صفحه عمومی جیگری می‌آید (عنوان پلن با رقم فارسی رندر می‌شود). تیترهای انگلیسی «Coverage Matrix» و «Jigari pricing» پنل فارسی شدند. Regression کامل روی MySQL 8.4.11 ایزوله برابر 136/136 با 1225 assertion و بدون Skip PASS شد؛ Pint 215 فایل و Build نیز PASS هستند.
