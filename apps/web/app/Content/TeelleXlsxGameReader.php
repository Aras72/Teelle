...
final class TeelleXlsxGameReader
{
    ...
    private function game(array $row): array
    {
        ...
        return [
            ...
            'metadata' => [
                ...
                'prep_time_minutes' => $this->number($this->value($row, 14)), 'space_required' => $this->decode($this->firstPart($this->value($row, 15)), 'space'),
                'noise_level' => $this->decode($this->firstPart($this->value($row, 16)), 'noise'), 'mess_level' => $this->decode($this->firstPart($this->value($row, 17)), 'mess'),
                ...
                'child_energy' => $this->decode($this->firstPart($this->value($row, 22)), 'energy'), 'caregiver_energy' => $this->decode($this->firstPart($this->value($row, 23)), 'energy'),
                'interaction_type' => $this->decode($this->firstPart($this->value($row, 24)), 'interaction'), 'caregiver_involvement' => $this->decode($this->firstPart($this->value($row, 25)), 'involvement'),
                'setup_complexity' => $this->decode($this->firstPart($this->value($row, 26)), 'setup'),
    ...
    private function decode(string $value, string $type): string
    {
        $maps = [
            ...
            'situation' => [
                'بین وعده‌های غذایی' => 'between-meals', 'بعد از غذا' => 'after-meal',
                'بین کارهای روزمره' => 'between-routines', 'قبل از خواب' => 'before-bed',
            ],
            'location' => [
