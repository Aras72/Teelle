<x-layouts.app title="{{ $article->exists ? 'ویرایش مطلب' : 'مطلب تازه' }}" description="فرم کامل مجله تیله">
    <div class="admin-shell teelle-container">
        ...
        <form class="admin-form admin-panel" method="post" enctype="multipart/form-data" action="{{ $article->exists ? route('admin.content.articles.update', $article) : route('admin.content.articles.store') }}">
            ...
            <fieldset><legend>نمایش در موتورهای جست‌وجو</legend><div class="admin-form-grid"><label>عنوان SEO<input class="teelle-input" name="seo_title" value="{{ old('seo_title', $article->seo_title) }}" maxlength="220"></label><label class="admin-field-wide">توضیح SEO<textarea class="teelle-input" name="seo_description" rows="3" maxlength="320">{{ old('seo_description', $article->seo_description) }}</textarea></label></div></fieldset>
