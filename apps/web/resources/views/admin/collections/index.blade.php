<x-layouts.app title="مدیریت مجموعه‌ها">
    <div class="admin-shell teelle-container">
        <header class="admin-heading"><div><p class="admin-kicker">Editorial</p><h1>مجموعه‌های بازی</h1></div><nav class="admin-actions"><x-ui.button href="{{ route('admin.content.collections.create') }}">پیش‌نویس تازه</x-ui.button><a href="{{ route('admin.content.index') }}">بازگشت به محتوا</a></nav></header>
        @if(session('status'))<p class="admin-notice" role="status">{{ session('status') }}</p>@endif
        <section class="admin-panel"><div class="admin-table-wrap"><table class="admin-table"><thead><tr><th>عنوان</th><th>Slug</th><th>وضعیت</th><th>بازی</th><th>عملیات</th></tr></thead><tbody>
